...
2. 회원가입(`/signup`) 또는 로그인(`/login`) — 이메일로 받은 6자리 인증번호로 본인 확인 후, 서버가 세션 토큰을 해시 저장하고 HttpOnly 세션 쿠키 발급 (비밀번호 없음)
...
- **회원 인증**: 비밀번호 없는 이메일 6자리 인증번호 방식(Resend 발송). 세션 토큰 해시 저장 + HttpOnly/Secure/SameSite 쿠키, 코드 10분 만료·1분 재발송 제한·5회 오답 제한 적용
